...
(function() {
	...
	app = angular.module('crucem-elit', [ 'ui.bootstrap','ngRoute', 'ngResource', 'ngTable', 'ngCookies','angularFileUpload', 'ngAnimate', 'toaster', 'ngFabForm', 'ngMessages' ]);
	...
	app.run(function ($rootScope, $http, $location, $cookieStore, $timeout) {
		...
		$rootScope.$on('$locationChangeStart', function (event, next, current) {
			...
			if (user !== undefined) {
				...
				else if ( $location.url().indexOf("/user/gyms/") > -1 && $location.url().replace("/user/gyms/", "").length > 0 && (!user.gym || user.gym.id === 0)) {
					event.preventDefault();
